...
const loginUser = asyncHandler(async (req,res) => {
    ...
    // set cookie and modified by server only cannot modified by frontend
    const options = {
        httpOnly: true,
        secure: true
    }
...
const refreshAccessToken = asyncHandler(async (req,res) => {
    ...
    try {
        ...
        const {accessToken, newRefreshToken: refershToken} = await generateAccessAndRefereshTokens(user._id)
...
const updateUserCoverImage = asyncHandler(async (req,res) => {
    ...
    if(!coverImageLocalPath) throw new ApiError(400,"Cover Iamge File is missing");
